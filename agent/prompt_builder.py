...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "Review/QA child rule: if `kanban_show()` lists an existing review "
    "child / pre-created review child, do not create another review card. "
    "Write durable handoff/comment/metadata on the parent, then complete/"
    "block it so the existing child can run. Origin ACK rule: workers "
    "write durable board results; the main operator/watchdog relays "
    "GO/BLOCK/NEED_MORE to the origin/return_to channel.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
